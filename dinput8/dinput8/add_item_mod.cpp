...
//
// __thiscall in MSVC: 'this' goes in ECX, remaining params on the stack.
// We model this as a normal function call where the first argument is the
// explicit 'this' pointer — MSVC will emit the correct __thiscall prologue
// when we declare the typedef with __thiscall.
// ---------------------------------------------------------------------------
...
//
//  Chain (32-bit absolute VA, module base 0x00400000):
//    pHeroPtrHolder = kHeroPtrPtr                 → DWORD (VA of CThing*)
//    pHeroThing     = *pHeroPtrHolder             → CThing*
//    pInventory     = *(CThing* + kInventoryOffset)
// ---------------------------------------------------------------------------
...
//
// This is a naive linear scan — replace with the game's own lookup function
// once you identify it (search the dump for a thiscall with one int param
// near the CThing vtable cluster).
//
// Returns nullptr if UID is 0 or no matching object is found.
// ---------------------------------------------------------------------------
void *FindCThingByUID(DWORD uid) {
  if (uid == 0)
    return nullptr;

  // TODO: replace with the real game lookup if you find it.
  // For now just cast the UID directly as a raw pointer so callers can detect
  // the "not implemented" case by checking for nullptr.
...
// Use GetAsyncKeyState: independent of message queues or hook chains which
// game engines sometimes disrupt.
// ---------------------------------------------------------------------------
...
void InstallAddItemMod() {
  Log("[AddItemMod] Installing AddItemToInventory mod (target VA=0x%08X).",
      kAddItemAddr);
